    ...
    <Card className="mb-10">
      <CardContent className="space-y-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{text.converterTitle}</h2>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {text.converterDescription}
            </p>
          </div>
          <select
            value={mode}
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div>
            <textarea
              ...
              placeholder={mode === 'yaml-to-json' ? text.pasteYaml : text.pasteJson}
            />
            <div className="mt-3 flex flex-wrap justify-end gap-2">
              ...
              <Button onClick={onConvert}>
                <ArrowLeftRight className="h-4 w-4" /> {text.convert}
              </Button>
            </div>
          </div>
          ...
          <div>
            <pre className="h-64 overflow-auto rounded-xl bg-gray-950 p-4 text-sm text-gray-100">
              <code>{output || text.convertedPlaceholder}</code>
            </pre>
            <div className="mt-3 flex flex-wrap justify-end gap-2">
